...
def consumidor():
    ...
    for message in consumer: 
        ...
        if "NPC_eliminado@" in mensaje: #me eliminaron de la partida
            #Obtengo la pos del eliminado
            posMovimiento = mensaje.split("@")[1].split(",")     #NPC_eliminado@{posMovimiento}
...
def npc():
    ...
    for message in consumer:
        ...
        if "inicio@" in mensaje or "mapa@" in mensaje or "info@" in mensaje or "jugadores@" in mensaje:
    ...
    if PARTIDA_DETECTADA:
        ...
        while True:
            ...
            producer.send(topico_partida, kafka_encriptar(f'NPC@{posActual[0]},{posActual[1]}@{POS_X},{POS_Y}@{nivel}'))
