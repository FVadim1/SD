...
def terminarPartida(JUGADORES_PARTIDA_ALIAS, producer):
    ...
    if len(JUGADORES_PARTIDA_ALIAS) == 1:
        ...
        producer.send(topico_partida, kafka_encriptar(f"info@¡'{alias_ganador}'|'{aiias_mapa_ganador}' ha ganado la partida!"))
...
def setLevel_Clima_Movimiento(alias,ciudad_movimiento : str,JUGADORES_PARTIDA_ALIAS):
    conexion_db = db()
    ##ABRO CONEXION
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
...
### NUEVOS MÉTODOS PRÁCTICA 3

def insertar_mapa_en_BaseDatos(mapa : Mapa):
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        #setter 
        conexion_db.insertar_mapa(mapa.getArray_insertarDB())

        conexion_db.closeCommunication()
    else:
        print("Error insertar_mapa_en_BaseDatos")


def borrar_mapa_en_BaseDatos():
    ##ABRO CONEXION CON LA BASE DE DATOS
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        #setter 
        conexion_db.delMapas()
        conexion_db.closeCommunication()
...
def getMapa_en_BaseDatos() -> list:
    ...
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        ...
        print("ERROR: no se ha podido establecer comunicación con la DB en getMapa_en_BaseDatos()")

    return m

def insertar_JugadoresPartida_en_BaseDatos(JUGADORES_PARTIDA_ALIAS : list):
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        #setter 
        conexion_db.insertar_jugadores_partida(JUGADORES_PARTIDA_ALIAS)

        conexion_db.closeCommunication()
    else:
        print("Error insertar_JugadoresPartida_en_BaseDatos")
...
def borrar_jugadoresPartida_en_BaseDatos():
    ##ABRO CONEXION CON LA BASE DE DATOS
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        conexion_db.delJugadoresPartida()
        conexion_db.closeCommunication() 
    else:
        print("ERROR: no se ha podido establecer comunicación con la DB en borrar_jugadoresPartida_en_BaseDatos() en auxiliares_Engine.py")

def insertar_ciudades_en_BaseDatos(ciudades : list):
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        #setter 
        conexion_db.insertar_ciudades_partida(ciudades)
        conexion_db.closeCommunication()
    else:
        print("Error insertar_ciudades_en_BaseDatos")

def getCiudades_en_BaseDatos() -> list:
    ##ABRO CONEXION CON LA BASE DE DATOS
    m = []
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        m = conexion_db.getCiudades()
        m = [tuple(l) for l in m] #convierto lista de listas en lista de tuplas (requerido en algunos métodos)
        ...
        m = [str(l) for l in m] #convierto lista de tuplas en listas de strings (en forma de tuplas)
    else:
        print("ERROR: no se ha podido establecer comunicación con la DB en getCiudades_en_BaseDatos() en auxiliares_Engine.py")

    return m

def borrar_ciudades_en_BaseDatos():
    ##ABRO CONEXION CON LA BASE DE DATOS
    conexion_db = db()   
    if(conexion_db.openCommunication(SERVIDOR_IP , USUARIO , CONTRASENYA, BASE_DE_DATOS) == True):
        
        conexion_db.delCiudades()
        conexion_db.closeCommunication() 
    else:
        print("ERROR: no se ha podido establecer comunicación con la DB en borrar_ciudades_en_BaseDatos() en auxiliares_Engine.py")


def BorrarPartidaAnterior_en_BaseDatos():
    borrar_mapa_en_BaseDatos() #borro los mapas que hay en la base de datos
    borrar_jugadoresPartida_en_BaseDatos() #borro los jugadores partida que hay en la base de datos
    borrar_ciudades_en_BaseDatos() #borro las ciudades que hay en la base de datos
